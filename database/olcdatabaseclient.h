...
class OLCDatabaseClient : public IDataBaseClient{
public:
	OLCDatabaseClient();
...
public:
	 ...
	 * @return error enumeration
	 */
	err_t AddCourseToMap(course_details_t &&course, bool db_write=true);
	 ...
	 * @return error enumeration
	 */
	err_t AddUserToMap(user_details_t &&user, bool db_write=true);
	 ...
	 * @return error enumeration
	 */
	err_t SubscribeCourse(course_id cid, usr_id uid, bool db_write=true);
...
private:
	...
	/*!
	 * Database class to Persist data in SqlDB
	 */
	 ...
	 * Database class to Persist data in SqlDB
	 */
	std::unique_ptr<FileStorage> mFileStorage;

...
OLCInteractionMgr& OLCInteractionMgr::operator=(const OLCInteractionMgr &other) {
...
err_t OLCInteractionMgr::DisplayCourseByTitle(str_t coursetitle) {
	auto course= OlcCourse<CourseDetails_t>();
	course.SetData(mDBmgr->GetCourseByTitle(coursetitle));
	return course.DisplayDetails();
}

err_t OLCInteractionMgr::DisplayCourseByAuthor(str_t author) {
	return mDBmgr->DisplayCoursesByAuthor(author);
}

err_t OLCInteractionMgr::DisplayAvgCoursePrice() {
	std::cout<<"\nThe Average course price is : "<<mDBmgr->GetAvgCoursesPrice()<<"\n";
	return NO_ERROR;
}

err_t OLCInteractionMgr::DisplayMinimumCoursePrice() {
	std::cout<<"\nThe Minimum course price is : "<<mDBmgr->GetMinimumCoursePrice()<<"\n";
	return NO_ERROR;
}
